...
Для AmneziaWG установщик выбирает совместимую ветку официального PPA: Focal для Debian 11, Jammy для Debian 12/Ubuntu 22.04 и Noble для Debian 13/Ubuntu 24.04+. Ключ репозитория скачивается отдельно и принимается только при совпадении полного fingerprint; устаревшие записи PPA, оставленные предыдущим установщиком, заменяются с резервной копией. После установки проверяются `awg`, `awg-quick`, загрузка модуля `amneziawg`, генерация ключей и создание временного сетевого интерфейса.
